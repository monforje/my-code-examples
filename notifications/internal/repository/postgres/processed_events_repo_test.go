...
func TestProcessedEventsRepo_Create(t *testing.T) {
	...
	event := &records.ProcessedEvent{
		...
		EventType:   "notification.email.verification_code.send",
...
func TestProcessedEventsRepo_GetByAggregateID(t *testing.T) {
	...
	e2 := &records.ProcessedEvent{
		EventID:     uuid.New().String(),
		EventType:   "notification.email.password_reset_code.send",
		AggregateID: aggregateID,
...
func TestProcessedEventsRepo_Update(t *testing.T) {
	...
	event.EventType = "notification.email.password_reset_code.send"
	...
	if got.EventType != "notification.email.password_reset_code.send" {
		t.Errorf("EventType = %v, want notification.email.password_reset_code.send", got.EventType)
	}
}
