...
func newE2EConfig(env *e2ehelpers.Environment) *config.Config {
	return &config.Config{
		...
		Features: config.FeaturesConfig{
			...
			CodeResendCooldown:     time.Millisecond,
			CodeResendWindow:       time.Minute,
			CodeResendMaxRequests:  10,
...
func waitHTTP(ctx context.Context, baseURL string) error {
	...
	for {
		request, err := http.NewRequestWithContext(ctx, http.MethodGet, baseURL+"/auth/me", nil)
		if err != nil {
			return err
		}
