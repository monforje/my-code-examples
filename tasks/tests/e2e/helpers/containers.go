// Package e2e_test_helpers предоставляет утилиты для e2e-тестов.
//
// Контейнеры:
//   - StartEnvironment: запуск PostgreSQL в testcontainers
//   - Shutdown: остановка и очистка всех контейнеров
//   - Reset: очистка БД между тестами
...
const (
	...
	postgresDB       = "tasks_e2e"
)

// Environment - окружение для e2e-тестов с PostgreSQL в контейнере.
...
// PgPool - пул соединений с тестовой БД.
func (e *Environment) PgPool() *pgxpool.Pool { return e.pgPool }

// StartEnvironment - запуск PostgreSQL контейнера и выполнение миграций.
...
// Shutdown - остановка всех контейнеров и закрытие соединений.
...
// Reset - очистка всех таблиц между тестами.
func (e *Environment) Reset(ctx context.Context) error {
	_, err := e.pgPool.Exec(ctx, `TRUNCATE tasks CASCADE`)
	if err != nil {
